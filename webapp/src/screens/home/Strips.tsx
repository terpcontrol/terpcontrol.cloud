      ...
      {due.map(({ card, task }) => (
        <li key={task.id}>
          <div className={styles.chip}>
            ...
            <Link to={`/log?task=${encodeURIComponent(task.id)}`} className={styles.done}>
              {t('home.strip.done')}
            </Link>
          </div>
        </li>
...
const dueLabel = (t: (key: string, options?: Record<string, unknown>) => string, task: DueTask, now: DateTime): string => {
