...
import { Device, DeviceClass, DeviceClassFirmwareStats, DeviceFirmware, FirmwareChannel, FirmwareFleetStats } from '@fg2/shared-types';
  ...
  public async getFirmwareVersions(): Promise<DeviceClassFirmwareStats[]> {
    ...
    const class_count = await Promise.all(
      classes.map(async deviceclass => {
        ...
        const versions: FirmwareFleetStats[] = await Promise.all(
