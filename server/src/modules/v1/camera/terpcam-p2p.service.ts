...
import { STILL_REQUEST, StillRequestPort } from './still-request';
...
export class TerpCamP2PService {
  constructor(
    ...
    @Optional() @Inject(STILL_REQUEST) private readonly protocol: StillRequestPort | null,
  ) {}
  ...
  /**
   * Ask the device for a still and resolve with the assembled JPEG. Keyed by the
   * controller rather than by the camera, because a controller pairs exactly one
   * Terp Cam and the command names none.
   */
  public captureViaController(deviceId: string): Promise<Buffer> {
    if (!this.protocol) {
      return Promise.reject(new Error('nothing is speaking to the devices, so no controller can be asked for a picture'));
    }
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      if (!this.protocol?.captureStill(deviceId)) {
        this.failPending(deviceId, new Error('the device is not connected to the broker'));
      }
    });
  }
