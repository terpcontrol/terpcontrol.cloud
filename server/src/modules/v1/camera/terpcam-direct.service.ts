 ...
 * password come off that camera's own record, written from what its controller
 * reported. A standalone camera is reached exactly the same way - there is no
 * controller in this path at all.
 *
...
/**
 * A camera as this path needs it: which one to look for, and what to log in
 * with. An empty secret means the camera still has the manufacturer's default.
 */
export interface P2PCamera {
  id: string;
  uid: string | null;
  secret: string | null;
}
...
export class TerpCamDirectService implements OnApplicationShutdown {
  ...
  private dropSession(cameraId: string): void {
  ...
  private touchSession(cameraId: string): void {
    const session = this.sessions.get(cameraId);
    if (!session) return;
    clearTimeout(session.idle);
    session.idle = setTimeout(() => this.dropSession(cameraId), SESSION_IDLE_MS);
    session.idle.unref?.();
  }

  /** Open a session (one rendezvous) or reuse the one already held. */
  private async session(camera: P2PCamera & { uid: string }) {
    ...
    try {
      ...
      const auth = authFor(camera.secret || DEFAULT_PASSWORD);
      ...
      this.touchSession(camera.id);
      logger.info(`[terpcam] ${camera.id}: session opened (one rendezvous, reused for later stills)`);
      return session;
    } catch (error) {
      try {
        socket.close();
   ...
   * and a P2P id on the camera's record. Where it cannot, the controller is not
   * a fallback but the only path there is, and a caller should not spend failed
   * attempts before taking it.
   */
  public canReach(camera: P2PCamera): boolean {
    return this.rendezvousHosts.length > 0 && !!camera.uid;
  }

  /** Pull one still as a ready JPEG, decoding the keyframe when there is one. */
  public async captureStill(camera: P2PCamera): Promise<Buffer> {
    const { data, h264 } = await this.capture(camera);
    if (!h264) return data;
    const jpeg = await this.stills.decodeKeyframeToJpeg(data);
    logger.info(`[terpcam] ${camera.id}: ${data.length}B keyframe -> ${jpeg.length}B jpeg`);
    return jpeg;
  }
   ...
   */
  private async readStill(camera: P2PCamera & { uid: string }): Promise<Buffer | null> {
    const session = await this.session(camera);
    clearInterval(session.alive);
    try {
      return await this.readKeyframe(session);
    } finally {
      if (this.sessions.get(camera.id) === session) {
        session.alive = this.startHeartbeat(session);
      }
    }
  }
   ...
   */
  public async capture(camera: P2PCamera): Promise<{ data: Buffer; h264: boolean }> {
    const uid = camera.uid;
    if (!uid) {
      throw new Error('this camera has no P2P id, so the server cannot look it up');
    }
    ...
    const attempts = this.sessions.has(camera.id) ? 2 : 1;
    for (let attempt = 1; ; attempt++) {
      try {
        const keyframe = await this.readStill({ ...camera, uid });
        if (!keyframe) throw new Error('no keyframe arrived');
        this.touchSession(camera.id);
        return { data: keyframe, h264: true };
      } catch (error) {
        this.dropSession(camera.id);
        if (attempt >= attempts) throw error;
        logger.info(`[terpcam] ${camera.id}: held session went stale, opening a new one`);
      }
    }
  }
