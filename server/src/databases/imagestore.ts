 ...
 * still read from there - see `data` in the model.
 */

/** `imagedata.files` / `imagedata.chunks`, next to the `images` metadata. */
const BUCKET_NAME = 'imagedata';

const bucket = () => new mongo.GridFSBucket(connection.db, { bucketName: BUCKET_NAME });
...
class ImageStore {
  ...
  public async upload(imageId: string, data: Buffer): Promise<void> {
    await pipeline(Readable.from(data), bucket().openUploadStreamWithId(fileId(imageId), imageId));
  }
  ...
  public async uploadFile(imageId: string, path: string): Promise<number> {
    await pipeline(createReadStream(path), bucket().openUploadStreamWithId(fileId(imageId), imageId));
  ...
  public read(imageId: string, range?: { start: number; end: number }): Readable {
    return bucket().openDownloadStream(fileId(imageId), range ? { start: range.start, end: range.end + 1 } : undefined);
  }
   ...
   */
  public async delete(imageIds: string[]): Promise<void> {
    if (imageIds.length === 0) {
      return;
    }

    const ids = imageIds.map(fileId);
    await connection.db.collection(`${BUCKET_NAME}.chunks`).deleteMany({ files_id: { $in: ids } });
    await connection.db.collection(`${BUCKET_NAME}.files`).deleteMany({ _id: { $in: ids } });
  }
}
